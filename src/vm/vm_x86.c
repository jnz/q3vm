...
static int Hex(int c)
{
    ...
    VMFREE_BUFFERS();
    Com_Error(VM_FAILED_TO_LOAD_BYTECODE, "Hex: bad char");
...
static void EmitRexString(uint8_t rex, const char* string)
{
#if idx64
    if (rex)
        Emit1(rex);
...
static void DoSyscall(void)
{
    ...
    if (vm_syscallNum < 0)
    {
        ...
        *ret    = savedVM->systemCall(savedVM, (intptr_t*)data);
#endif
    }
    else
    {
        ...
        case VM_BLOCK_COPY:
            if (vm_opStackOfs < 1)
                Com_Error(VM_BLOCKCOPY_OUT_OF_RANGE,
...
int ConstOptimize(vm_t* vm, int callProcOfsSyscall)
{
    int v;
    int op1;
...
void VM_Compile(vm_t* vm, const vmHeader_t* header)
{
    ...
    buf       = Com_malloc(maxLength, vm, VM_ALLOC_TYPE_MAX);
    jused     = Com_malloc(jusedSize, vm, VM_ALLOC_TYPE_MAX);
    code      = Com_malloc(header->codeLength + 32, vm, VM_ALLOC_TYPE_MAX);
    ...
    Com_Memcpy(code, (uint8_t*)header + header->codeOffset, header->codeLength);
    ...
    for (pass = 0; pass < 3; pass++)
    {
        ...
        while (instruction < header->instructionCount)
        {
            ...
            {
                VMFREE_BUFFERS();
                Com_Error(VM_FAILED_TO_LOAD_BYTECODE,
            ...
            case OP_LOAD4:
                    ...
                    buf[compiledOfs - 1] == 0x9F)
                {
                    ...
                    Emit4((intptr_t)vm->dataBase);
#endif
                    EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr
                                                             // [edi + ebx * 4],
                                                             // eax
                    break;
                }
    ...
    if (vm->codeBase == MAP_FAILED)
        Com_Error(VM_FAILED_TO_LOAD_BYTECODE,
                  "VM_CompileX86: can't mmap memory");
#elif _WIN32
    ...
    if (!vm->codeBase)
        Com_Error(VM_FAILED_TO_LOAD_BYTECODE,
                  "VM_CompileX86: VirtualAlloc failed");
#else
    vm->codeBase = malloc(compiledOfs);
    if (!vm->codeBase)
        Com_Error(VM_FAILED_TO_LOAD_BYTECODE,
                  "VM_CompileX86: malloc failed");
#endif
...
extern uint8_t qvmcall64(int* programStack, int* opStack,
                         intptr_t* instructionPointers, uint8_t* dataBase);
#endif
...
int VM_CallCompiled(vm_t* vm, int* args)
{
    ...
    *opStack   = 0xDEADBEEF;
    ...
    if (opStackOfs != 1 || *opStack != (int)0xDEADBEEF)
    {
        Com_Error(VM_STACK_ERROR, "opStack corrupted in compiled code");
    }
    if (programStack != stackOnEntry - (8 + 4 * MAX_VMMAIN_ARGS))
        Com_Error(VM_STACK_ERROR, "programStack corrupted in compiled code");
